---
name: chatgpt-ads-workbook-builder
description: Assembles the three final deliverables for the ChatGPT Ads Bulk Builder agent — the campaign_workbook.xlsx (bulk upload spreadsheet matching OpenAI's template structure), context-hints-to-paste.docx (rich-formatted Word doc with context hints per ad group), and setup-checklist.docx (rich-formatted Word doc with budget plan, bid recommendations, image hosting instructions, and post-upload checklist). Use this skill after the copywriter has produced the campaign content. Loads xlsx and docx skill conventions for file construction. Output goes to outputs/ folder.
---
